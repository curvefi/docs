---
id: crvusd
title: crvUSD Risk Disclaimer
sidebar_label: crvUSD Risks
---
...
Collateralized debt positions are managed through arbitrage opportunities: if the collateral's price decreases, the system sells collateral in ‘soft-liquidation mode’. If the collateral price increases, the direction of conversion can reverse, but losses from earlier conversions do not reverse. This algorithm adjusts the assets backing each crvUSD position and avoids liquidation at one specific price, potentially giving borrowers more time to react. It does not eliminate collateral-volatility risk or guarantee recovery. Additional information can be found on [LLAMMA Overview](/developer/crvusd/amm).
